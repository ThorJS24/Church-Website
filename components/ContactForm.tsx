...
import { Send, User, Mail, Phone, MessageSquare, MapPin, Calendar, Users, Heart } from 'lucide-react';
import { SecurityManager } from '@/lib/security';
import CountryCodeSelect from '@/components/CountryCodeSelect';

interface ContactFormProps {
  type?: 'general' | 'prayer' | 'volunteer' | 'event';
  title?: string;
}

export default function ContactForm({ type = 'general', title = 'Contact Us' }: ContactFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!SecurityManager.validateInput(formData.name) || !formData.name.trim()) {
      newErrors.name = 'Name is required and contains invalid characters';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSubmitted(true);
        setFormData({
          name: '', email: '', phone: '', countryCode: '+1', subject: '', message: '',
          requestType: type, urgency: 'normal', preferredContact: 'email',
          ministry: '', availability: '', skills: ''
        });
      } else {
  ...
  const handleInputChange = (field: string, value: string) => {
    setFormData({ ...formData, [field]: SecurityManager.sanitizeInput(value) });
  };
    ...
    >
      <div className="text-center mb-8">
        <div className="w-16 h-16 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4">
          {type === 'prayer' ? <Heart className="w-8 h-8 text-white" /> :
           type === 'volunteer' ? <Users className="w-8 h-8 text-white" /> :
           type === 'event' ? <Calendar className="w-8 h-8 text-white" /> :
           <MessageSquare className="w-8 h-8 text-white" />}
        </div>
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{title}</h2>
        <p className="text-gray-600 dark:text-gray-300">
          {type === 'prayer' ? 'Share your prayer requests with our community' :
           type === 'volunteer' ? 'Join our ministry and make a difference' :
           type === 'event' ? 'Get information about upcoming events' :
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={
              ...
              type === 'volunteer' ? 'Tell us about your interests and how you\'d like to help...' :
              'Your message...'
            }
          />
